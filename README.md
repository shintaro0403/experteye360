...
**PDF 出力** — **最小実装済み**（`shared/src/pdfExport.ts` / `pdfExport.test.ts`、管理者回答詳細からの PDF ダウンロード UI 代表テスト、開ける最小 PDF 構造、日本語・英語・数字と最低限の装飾テストは Green。実 PDF 目視確認は未）
...
**自動テスト** — **Vitest**（`npm test` — 現状 14 files / 83 tests Green）
...
- 現状: `shared/src/pdfExport.ts` で生成用ペイロードを作り、開ける最小 PDF 構造を持つ `Uint8Array` を返す実装まで完了。日本語・英語・数字を UTF-16BE text として出し、見出し背景や区切り線を持つ。管理者回答詳細から PDF ダウンロードを実行する代表 UI テストも Green。
- 未実装: 実 PDF の目視確認。
- 目標: 回答済み 1 件につき 1 ファイルを、ブラウザ上で **jsPDF** により生成する。
- PDF に含める予定の項目:
  1. **どのシーンか**（表示名等）
...
- **回答済み**の結果を **PDF でエクスポート**（1 人 1 ファイル・jsPDF。§6。共有ロジックと管理者 UI は最小実装済み）

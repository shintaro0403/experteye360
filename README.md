...
**PDF 生成ロジック** — **最小実装済み**（`shared/src/pdfExport.ts` / `pdfExport.test.ts` は Green。管理者ダウンロード UI、jsPDF 本レイアウト、目視確認は未）
...
**現状: 一覧・詳細は実装済み。PDF 生成ロジックは最小実装済み。管理者画面からのダウンロード UI は未実装。**
...
**PDF ダウンロード（生成ロジック最小実装済み・UI 未実装）**

- 現状: `shared/src/pdfExport.ts` で生成用ペイロードを作り、空でない `Uint8Array` を返す最小実装まで完了。
- 未実装: ブラウザ上の **jsPDF** 本レイアウト、管理者画面からのダウンロード UI、実 PDF の目視確認。
...
- **回答済み**の結果を **PDF でエクスポート**（1 人 1 ファイル・jsPDF。§6。共有ロジックは最小実装済み、UI は未）

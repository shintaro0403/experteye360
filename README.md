...
プロダクトとしては、受講者が複数の判断軸から選び、**優先順位を付けて並べる**ことで「何を重視して判断したか」を記録する想定である。

**現行の研修用 Web（プロトタイプ）**では、気づき・共有と同様に **カードは 1 枚だけ選ぶ（単一選択）** 実装としている。並べ替え UI は未実装で、仕様の整理は [docs/TECHNICAL-SPEC.md §4.3.10](docs/TECHNICAL-SPEC.md#4310-判断基準readme-のゴールとプロトタイプの違い) を参照。
...
講師側で、研修中または研修後に次を確認できるようにする。

- 受講者ごとの **回答状況**（**回答中**／**回答済み**）の一覧
- **回答済み**の受講者について、必要に応じて **PDF でエクスポート**する（帳票・配布用）
...
**回答済み**の回答データを PDF にまとめ、比較・共有・保管に使えるようにする（ダッシュボードからのエクスポート想定。詳細は技術仕様 F7）。
...
- 講師・管理者向けダッシュボードで、**回答状況**（回答中／回答済み）を確認できるようにする
- **回答済み**の結果を **PDF でエクスポート**できるようにする
- 研修結果からOJTで確認すべきポイントを整理する
...
- 共有・行動カード
- 受講者回答の保存
- 講師・管理者ダッシュボード（回答状況・回答済みの PDF エクスポート）
- OJT確認項目の整理（§7）
...
4. **`?client=` と研修コード（必須）** — 受講者の入室は **研修コード入力のみ**（URL に `room` を載せない）。GAS が `rooms` の `accessCodeHash` で検証し、通過後に `roomId` を確定する。以降の API では `client` + `room` が必須（正本は `rooms` シート）

受講者・管理者のいずれも、**研修コード**で当該研修回に紐づけてから操作する（詳細は [docs/SPREADSHEET-DATA.md §2.3](docs/SPREADSHEET-DATA.md#23-研修回ルームroomid)）。

...
| 3DVista 埋め込み | **専用 iframe 1 本**（参加者用レイアウト・同一ツアー内） | **別 iframe 1 本**（管理者用レイアウト・同一ツアー内の別配置） |
| 埋め込み URL | 受講者エントリのみ | 管理者エントリのみ |
| 25% 高さ制約 | **適用する**（パノラマ下帯） | **受講者帯には載せない**（管理者側は **幅40%×高さ100%** のフレーム想定・§3.2.2） |
...
| 設置場所 | 参加者ボタンから開く参加者用レイアウト（シーン／パネル） | 管理者ボタンから開く管理者用レイアウト（**受講者 25% 帯とは別配置**。同一ツアー内の別シーン等） |
...
| --- | --- |
| 受講者 25% 帯 | **使用しない** |
| サイズ | 管理者用ツアー画面のビューポートに対して **幅 40% × 高さ 100%**（パノラマ約 60% を確保する前提。README と同一） |
...
| 項目 | 内容 |
| --- | --- |
| ローカル確認 | `admin-web/public/embed-preview.html` は外側に **40vw** のモック枠を置き、本番の **40%×100%** iframe を近似する。アプリ本体は iframe **100%×100%** 前提（README「管理者の見た目確認」と同じ URL） |
...
README の機能章（§1〜§8）に対応する実装単位の整理。機能 ID は **F1、F3〜F8**（未採用の ID は欠番とする）。
...
| F4 | 判断基準カード | 各ラウンド **1 枚のみ単一選択**。`criteriaOrdered` は将来の並べ替え拡張用（§4.3.10） |
| F5 | 共有・行動カード | **単一選択**（1 枚） |
| F6 | 確信度 | **必須**。1〜5 の 5 ボタン。ラベル文言は実装（`@shared/confidence` 等）を正とする |
| F7 | 講師・管理者ダッシュボード | **回答状況**（回答中／回答済み）の一覧、**回答済み**からの **PDF エクスポート**。フィルタ・リアルタイム性（**要決定**） |
...
- `next` で遷移（README §3）。README では複数カードの **優先順位付け（並べ替え）** を最終的に想定しているが、**現行プロトタイプは気づき・共有と同様に 1 枚のみ単一選択**（`selectSingle`）。選択は `criteriaOrdered` に 0 または 1 件で保持する。
- 将来、ドラッグ並べ替え等で複数枚＋順序を保存する場合は §4.3.10 のとおり `criteriaOrdered` を拡張する。
...
| 項目 | 説明 |
| --- | --- |
| 確信度・送信 | 設問 5 の一言メモの次。**確信度は必須**（§4.3.7）。その後、送信確認 → 保存。 |
...
| ナビ中央（確信度 step） | タイトル**右**に `（必須）`（`.p-step-meta__badge--required`） |
...
#### 4.3.10 判断基準：README のゴールとプロトタイプの違い

| 区分 | 内容 |
| --- | --- |
| README（プロダクトのゴール） | 複数の判断基準カードから選び、**優先順位を付けて並べる**イメージ（§3） |
| 現行プロトタイプ | **1 枚だけ選ぶ**単一選択。`criteriaOrdered` は要素 0〜1 件 |
| 将来 | 並べ替え UI を入れたら `criteriaOrdered` に複数＋順序を入れ、バリデーションを「1 枚以上」等に変更する |
...
3. **`roomId`（研修コードで確定・必須）** — 同一 `clientId` 内の研修回分離。受講者の入室は **研修コード入力のみ**（URL に `room` を載せない方針。検証後に `roomId` を保持し API では `room` 必須）
...
flowchart TB
  subgraph vista["3DVista（ホスト・同一ツアー内で参加者／管理者レイアウトを切替）"]
    ...
    A_iframe["iframe #2 管理者<br/>admin/?client=…&token=…"]
  end
...
| 版 | 日付 | 内容 |
| --- | --- | --- |
| 1.1 | 2026-05-20 | 3DVista を同一ツアー内の参加者／管理者ボタン前提に整理。管理者 iframe **40%×100%**・§3.2.4。確信度 **必須**。入室は **研修コード**（§5.0）。README 節番号と §4.3 の整合。機能表から未採用の行を削除 |

...
- [0. 現状の問題と解決策（優先順）](#0-現状の問題と解決策)
  - [0.1 現状の問題（localStorage のみ・API 未実装時）](#01-現状の問題)
  - [0.2 解決策（実装の優先順）](#02-解決策)
  - [0.3 実装フェーズ（目安）](#03-実装フェーズ)
- [1. 方針](#1-方針)
- [1.1 データの流れ（図解）](#11-データの流れ)
- [2. 複数クライアント（テナント）](#2-複数クライアント)
  - [2.1 識別子 clientId](#21-識別子-clientId)
  - [2.2 ブック構成（採用）](#22-ブック構成)
  - [2.3 研修回（ルーム）roomId](#23-研修回roomId)
- [3. シート定義](#3-シート定義)
  - [3.1 マスターブック — clients シート](#31-マスターブック-—-clients-シート)
  - [3.2 クライアント用ブック — シート一覧](#32-クライアント用ブック-—-シート一覧)
  - [3.3 settings シートの列](#33-settings-シートの列)
  - [3.4 rooms シートの列](#34-rooms-シートの列)
  - [3.5 responses シートの列](#35-responses-シートの列)
  - [3.6 auditlogs シートの列](#36-auditlogs-シートの列)
...
- [7. 将来: PostgreSQL 等への移行](#7-将来-PostgreSQL-等への移行)
- [8. 制限・運用上の目安](#8-制限運用上の目安)
- [9. 改訂履歴](#9-改訂履歴)
...
- `localStorage` 実装（`shared/src/storage.ts`）は **単体テスト・オフライン用**として残す
- 結合確認は **本番近似** — 研修コード・管理者コードの入室を省略しない（[TEST-DESIGN.md §1.5](./TEST-DESIGN.md#15-入室マルチテナント)）
...
**役割** — UI のみ。直接スプレッドシートには触れない

#### `storage.ts` 窓口

**役割** — 本番・開発で **同じ関数シグネチャ**（Postgres 移行時も維持）

#### GAS

**役割** — 認可・`client` 解決・シート行の読み書き

#### スプレッドシート

**役割** — **唯一の共有正本**（受講者・管理者・端末をまたぐ）
...
**内容** — `clientId` + `roomId`（**研修コード検証後**に確定）

#### 粒度

**内容** — 1 行 = `ParticipantSubmission` 1 件（`submission_json` に `rounds[5]`）

#### 更新

**内容** — 受講者は **追記のみ**。削除・一括クリアは管理者 + `audit_logs`
...
**誰がやるか** — 運用者がスプレッドシートをコピー・リネームし、マスター `clients` に 1 行追加
**向いている場面** — 初回検証・クライアント数が極少

#### **B. 自動（推奨）**

**誰がやるか** — GAS がテンプレートを複製し、名前変更・初期データ投入・マスター登録まで実施
**向いている場面** — 複数クライアントを増やす本番運用
...
https://{host}/admin/?client=client-a
```

（管理者の **管理者コード** は URL に載せず、画面で入力。API 呼び出し時に `token` として送る。）

**受講者 UI**

- **名前・所属より前**に **研修コード入力**（必須）→ `POST rooms/verify` で `accessCodeHash` 照合 → 成功時のみ `roomId` を `sessionStorage` に保持しプロフィール欄を表示
- 不一致時は **「正しい研修コードを入力してください」**（文言固定）
